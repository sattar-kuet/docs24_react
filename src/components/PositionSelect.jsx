...
import { useGetPositionsQuery } from "../app/features/data/dataApi";

export default function PositionSelect({ control, errors }) {
  ...
  const options = useMemo(() => {
    if (positionData?.result?.data?.length > 0) {
      return positionData?.result?.data?.map((value) => {
